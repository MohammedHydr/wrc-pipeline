...
| I2 | Orchestrator with ingestion/transformation as separate dependent tasks | `orchestration/wrc_dagster/definitions.py` — `scrape_landing_zone >> transform_to_curated >> enrich_decisions` (Dagster job), **monthly-partitioned** via `wrc_monthly_config` (backfill grid; one independently retryable run/subprocess per month); `wrc_monthly_incremental` schedule materializes the previous month's `partition_key` | README §5 trigger instructions; `tests/test_orchestration.py` (partition→config mapping incl. leap month, schedule targeting, config validation); `dagster job execute` run logs |
...
| D2 | Landing Zone never deleted/updated | Append-only keys (new hash → new key), insert-only version records, transform never writes to landing | `ARCHITECTURE.md` §Deduplication; `tests/test_integration_idempotency.py` (changed content appends a new version, never overwrites) |
| D3 | Found vs scraped accounting; every missing record logged with reason | `run_stats` per (partition, body): `found`, `scraped`, `unchanged`, `failed[]` (URL, identifier, error, HTTP status), plus `parse_failures[]` (unusable listing cards), `skipped` (fast-mode), `docs_enqueued` → `records_unaccounted` (silent losses). `parse_document()` wraps callbacks so an unhandled exception is a recorded `stage=parse` failure, not a lost record. If the result-count text fails to parse, `_follow_next_page()` degrades to walking the pager's next link (loop-guarded) so pages 2+ are still crawled while the partition stays flagged `count_parse_failed`. Failures also land in the `failed_documents` **dead-letter ledger** (`_persist_failure_ledger()`: unique row per (url, identifier, partition, body), `retry_count` incremented per run, auto-resolved when a re-run succeeds; replay = re-run the partition) | Summary reconciliation `found == scraped + failed + parse_failed + skipped + listing_at_risk + unaccounted`; `tests/test_spider.py` covers clean / malformed-card / silent-loss / fast-mode / shortfall cases plus `test_count_parse_failure_falls_back_to_next_link_pagination`, `test_fallback_pagination_*`, `test_parsed_count_uses_numeric_fanout_not_next_link`, `test_ledger_rows_*` |
